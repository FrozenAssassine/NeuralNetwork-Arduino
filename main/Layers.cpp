...
void OutputLayer::Train(const float *desiredValues, float learningRate)
{
  ...
  for (int idx = 0; idx < this->Size; idx++)
  {
    ...
    if (this->activationKind == Sigmoid || this->activationKind == Softmax)
    {
      ...
      gradZ = this->Errors[idx];
    }
    else
    {
      // MSE or other loss
      gradZ = this->Errors[idx] * ActivationDeriv(this->NeuronValues[idx], this->activationKind);
    }
    ...
    for (int j = 0; j < this->PreviousLayer->Size; j++)
    {
      this->Weights[weightIndex + j] += learningRate * gradZ * this->PreviousLayer->NeuronValues[j];
    }

    this->Biases[idx] += learningRate * gradZ;
  }
}
